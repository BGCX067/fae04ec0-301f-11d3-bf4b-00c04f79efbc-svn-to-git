...
function DataReader (config) {
        ...
        row.find('Cell').each(function(index) {
            ...
            // Not a new Entry
                ...
                // colIndex now must be cellMeta
                    ...
                    // This is a special case (Office 2013 project)
                    // new row is igrnore cell and it's a child of root
                    var indexObjHander = getColumnIndexByObjHander(obj);
                    if(indexObjHander == colIndex - 1){
                        obj = dataObject[dataObject.length - 1];
                        className = 'Question';
                    }
            ...
            var isAnser = false; // special case for Office 2013
            if(ignore) treeNoteIndex += ignore;
            if(treeNoteIndex > maxCell){
                treeNoteIndex -= ignore - 1;
                isAnser = true;
            }
              
            var isQuestion = treeNoteIndex == maxCell;
            setTreeNote(objHandler, treeNoteIndex-1, isQuestion, isAnser);
        });
    }
     ...
     */
    function setTreeNote(objectHandler, index, isQuestion, isAnser){
        if(isQuestion==true) index -= 1;
        if(setting.treeNoteList[index] && objectHandler!=undefined){
            var note = setting.treeNoteList[index];
            var text = (isQuestion==true || (isAnser != undefined && isAnser == true)) ? 
                // Support treeNote, never show question - Office 2013
                (objectHandler.Parent.ignore != undefined) ? objectHandler.Parent.Text : objectHandler.Parent.Parent.Text  
                : objectHandler.Text;
    ...
    function getColumnIndexByObjHander(obj){
        var colIndex = 0;
        var getColIndex = function(o){
            if(o.Parent){
                colIndex += o.Parent.ignore || 0;
                o = o.Parent;
                return getColIndex(o);
            }
            else return colIndex;
        }
        return getColIndex(obj);         
    }
