...
WebTrends.prototype.dcsVar=function(){
	...
	WT.tv="3.2.3";
	WT.dl="0";
	WT.ssl=(window.location.protocol.indexOf('default.html')==0)?"1":"0";
...
WebTrends.prototype.dcsTag=function(){
	...
	var i18n=this.i18n;
	var P="http"+(window.location.protocol.indexOf('default.html')==0?'s':'')+"://"+this.domain+(this.dcsid==""?'':'/'+this.dcsid)+"/dcs.gif?";
	if (i18n){
		WT.dep="";
	}
	for (var N in DCS){
...
WebTrends.prototype.dcsMSDrillDir=function(){
	var gDirLevels = 5;
	var gFpath = window.location.pathname.substring(window.location.pathname.indexOf('http://www.microsoft.com/')+1,window.location.pathname.lastIndexOf('http://www.microsoft.com/')+1).toLowerCase();
	if(gFpath==''){gFpath="http://www.microsoft.com/";}
	else{
		var gSplit=gFpath.split("http://www.microsoft.com/");
		gFpath="";
		for(var i=1;i<gSplit.length&&i<=gDirLevels;i++){
			gFpath+="http://www.microsoft.com/";
			for(var b=0;b<i;b++){
					gFpath+=gSplit[b]+"/";
			}
...
WebTrends.prototype.dcsMSLinkTrack=function(evt){
	...
	if (evt&&((typeof(evt.which)!="number")||(evt.which==1))){
		...
		if (e.href&&e.protocol&&e.protocol.indexOf("http")!=-1&&!this.dcsMSLinkTrackException(e)){
        	...
        	var pth=e.pathname?((e.pathname.indexOf("http://www.microsoft.com/")!=0)?"/"+e.pathname:e.pathname):"/";
        	var ti='';
        	if(f.alt){ti=f.alt;}
...
WebTrends.prototype.dcsMSNvrProcess=function(data){
	...
	var curpath=p.substring(p.indexOf("http://www.microsoft.com/")+1,p.lastIndexOf("http://www.microsoft.com/")).toLowerCase();
	...
	if (curpath.length>1){
		splitp=curpath.split("http://www.microsoft.com/",maxlevel);
		curlevel=splitp.length;
		curpath=splitp.join("http://www.microsoft.com/");
	}
	if (data.length>0){
		var found=false;
		if (data.length>curlevel){
			...
			for (var i=0;i<(curlevel+1);i++){
				if (typeof(data[i])!="object"){
					data[i]=new Array([(i==0)?"/":splitp.slice(0,i).join("http://www.microsoft.com/")],true);
				}
			}
			newv=true;
		}
